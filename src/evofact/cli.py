...
def build_parser():
    ...
    adversarial.add_argument("--samples", help="normalized Sample JSONL including evidence snapshots")
...
async def _run(args):
    ...
    if args.command in {"dry-run", "test", "evolve", "validate", "ablation", "report"} and (root / config.skill_store / "active.json").exists():
    ...
    if args.command=="evolve":
        ...
        for proposal,decision in zip(result["proposals"],result["gate_decisions"]):
            if decision.disposition == "active":
                ...
                repo.commit_bank(apply_candidate(bank, proposal), run_id="fixed-" + proposal.proposal_id + "-" + skillbank_fingerprint(bank), baseline=bank, audit={"proposal": asdict(proposal), "decision": asdict(decision)})
                continue
            for skill in proposal.candidate_skills:
                repo.save(replace(skill,status=SkillStatus.PARETO if decision.disposition=="pareto" else SkillStatus.CANDIDATE),decision.disposition)
        return result
    if args.command=="meta-evolve":
        rows=fixture_meta_samples() if all_samples is None else all_samples; final_domains=tuple(sorted(x.strip() for x in args.final_test_domains.split(",") if x.strip()))
        overrides={}
    ...
    if args.command == "adversarial-evolve":
        ...
        if args.limit:
            raise ValueError("adversarial-evolve preserves complete evidence groups; --limit is unsupported")
        if args.samples:
        ...
        outcome = await adversarial_runner.run(rows, final_test_domains=final,
                                              resume=args.resume, evaluation_only=args.evaluation_only)
        paths = write_adversarial_report(root / config.output_dir, outcome, adversarial_runner.audit, config.generation)
        return {"mode": args.command, "run_id": outcome.run_id, "mock_results": outcome.mock_results,
                "episodes": len(outcome.episodes), "proposer": config.generation.proposer,
                "generator_calls": sum(e["generator_calls"] for e in adversarial_runner.audit.values()),
                "accepted_samples": sum(e["metrics"]["accepted"] for e in adversarial_runner.audit.values()),
                "committed_snapshots": outcome.committed_snapshots, "reports": paths}
    ...
    if args.command=="report":
        ...
        for offset in range(3):
            seeded=ExperimentRunner(replace(config,seed=config.seed+offset),root); seeded.skills=runner.skills; _,result=await seeded.run(report_rows); runs.append(result.aggregate_metrics)
        summary=summarize_runs(runs); _,grouped=await runner.run(report_rows); repository=SkillRepository(root/config.skill_store)
        payload={"metrics":{k:v["mean"] for k,v in summary.items()},"manifest_id":manifest.manifest_id if manifest else "fixture","mock_results":config.backend == "mock","multi_seed":summary,"domain_metrics":grouped.domain_metrics,"temporal_metrics":grouped.temporal_metrics,"paired_gate":[],"skill_evolution_curve":skill_evolution_curve(repository.history()),"completed":True}
