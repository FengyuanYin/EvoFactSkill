...
def compute_metrics(rows: list[SampleEvaluation]) -> dict[str,float]:
    n=len(rows); covered=[r for r in rows if r.predicted in LABELS]; correct=sum(r.gold==r.predicted for r in rows)
    f1s=[]
    for label in LABELS:
    ...
    return {"n":float(n),"accuracy_all":correct/n if n else 0,"macro_f1_all":sum(f1s)/2,"coverage":len(covered)/n if n else 0,"covered_accuracy":covered_correct/len(covered) if covered else 0,"selective_risk":1-covered_correct/len(covered) if covered else 1,"mean_cost":sum(r.cost for r in rows)/n if n else 0}

def grouped_metrics(rows: list[SampleEvaluation], field: str) -> dict[str,dict[str,float]]:
    groups=defaultdict(list)
    for row in rows: groups[str(getattr(row,field) or "unknown")].append(row)
