...
class InferenceRuntime:
    def __init__(self,backend,router:SkillRouter,skills:list[SkillSpec],utilities=None): self.backend=backend; self.router=router; self.skills=skills; self.utilities=utilities or {}
    async def infer(self,sample:Sample,budget:RunBudget=RunBudget())->InferenceTrace:
        public=sample.public_view(); decision=self.router.select(public,self.skills,self.utilities,budget); by_id={s.skill_id:s for s in self.skills}; reports=[]; usage=UsageRecord(); errors=[]
        ...
        return InferenceTrace(trace_id,public,decision,tuple(reports),result.value,{s.skill_id:s.version for s in self.skills},aggregated_evidence=aggregate_evidence(tuple(reports)),usage=usage,errors=tuple(errors))

def _merge(a:UsageRecord,b:UsageRecord)->UsageRecord:
    return UsageRecord(a.calls+b.calls,a.prompt_tokens+b.prompt_tokens,a.completion_tokens+b.completion_tokens,a.latency_ms+b.latency_ms,a.estimated_cost+b.estimated_cost)
