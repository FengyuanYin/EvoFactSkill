...
只根据每项给定的 evidence 和 label_contract 判断 text 的核心断言，不使用常识猜测、
不联网，不把生成者标签或策略作为结论。证据足够时从 allowed_labels 选择一个标签；无法
区分时返回 status=INDETERMINATE 且 predicted_label=null。INDETERMINATE 不是业务标签。
只返回 {"reviews": [{"sample_id": "...", "status": "CLASSIFIED|INDETERMINATE",
"predicted_label": "allowed label or null", "reason": "具体证据依据"}]}，每项恰好一条。
"""
