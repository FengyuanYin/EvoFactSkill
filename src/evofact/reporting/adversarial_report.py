...
def write_adversarial_report(path, outcome, audit, config):
    ...
    for identifier, episode in audit.items():
        ...
        target.write_text("".join(json.dumps(s, ensure_ascii=False) + "\n" for s in episode["samples"]), encoding="utf-8")
        sample_paths[identifier] = str(target)
    payload = {"schema_version": "adversarial_report_v2", "run_id": outcome.run_id,
               "mock_results": outcome.mock_results, "proposer": config.proposer,
               "generator": "one-shot trace-conditioned LLM",
               "episodes": audit, "sample_files": sample_paths, "detector_reports": paths}
    json_path = path / "adversarial_report.json"
    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = ["# Trace 驱动的单次 LLM 样本生成", "",
    ...
    lines += ["| Episode | 生成数量 | 审核通过 | 有效率 | 主系统错误率 |",
              "|---|---:|---:|---:|---:|"]
    for identifier, episode in audit.items():
        m = episode["metrics"]
        lines.append(f"| {identifier} | {m['generated']} | {m['accepted']} | {m['validity']:.3f} | {m['difficulty']:.3f} |")
    md = path / "adversarial_report.md"
    md.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return {**paths, "adversarial_json": str(json_path), "adversarial_markdown": str(md),
