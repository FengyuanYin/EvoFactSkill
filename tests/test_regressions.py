...
from evofact.core.models import EvolutionProposal, SampleEvaluation, SkillScope, SkillStatus
from evofact.core.models import RunBudget, TransferUtility, Trigger
    ...
    def test_scopes_are_hard_filters_for_all_strategies(self):
        ...
        for strategy in ("static", "random", "all-experts", "utility-aware"):
            with self.subTest(strategy=strategy):
                route = SkillRouter(strategy).select({"domain": "finance"}, [skill], {}, RunBudget())
...
class LifecycleRegressionTests(unittest.TestCase):
    ...
    def test_split_and_merge_replace_all_targets(self):
        ...
        self.assertEqual(set(s.name for s in apply_candidate(bank, merge)), set(s.name for s in self.bank))
    ...
    def test_missing_frozen_duplicate_targets_rejected(self):
        proposals = [EvolutionProposal("x", Op.EDIT, "bad", ("missing",), (self.a,)),
                     EvolutionProposal("x", Op.ADD, "bad", (), (self.old,))]
        for proposal in proposals:
            with self.assertRaises(ValueError):
                apply_candidate(self.bank, proposal)
        with self.assertRaisesRegex(ValueError, "frozen"):
            apply_candidate([replace(self.old, status=SkillStatus.FROZEN)], EvolutionProposal("x", Op.EDIT, "bad", (self.old.name,), (self.a,)))
...
class ResumeRegressionTests(unittest.TestCase):
    def test_fingerprints_cover_labels_resources_and_triggers(self):
        ...
        self.assertNotEqual(data_fingerprint(rows), data_fingerprint([replace(rows[0], label="FAKE"), *rows[1:]]))
        bank = ExperimentRunner(AppConfig(), ROOT).skills
        for change in ({"resources": {"references/a.md": "new"}}, {"triggers": (Trigger("text", "cue"),)}):
            self.assertNotEqual(skillbank_fingerprint(bank), skillbank_fingerprint([replace(bank[0], **change), *bank[1:]]))

    def test_resume_rejects_changed_budget_or_domain_policy(self):
        with tempfile.TemporaryDirectory() as directory:
            config = load_config(ROOT / "configs/demse_dry_run.yaml")
            config = replace(config, meta_learning=replace(config.meta_learning, checkpoint_path=Path(directory) / "checkpoint.json"))
            asyncio.run(MetaEvolutionRunner(config, ROOT).run(fixture_meta_samples(), final_test_domains=("outer_holdout",), evaluation_only=True))
            for changed, final in [(replace(config, max_skills_per_item=1), ("outer_holdout",)), (config, ("finance",))]:
                with self.assertRaisesRegex(ValueError, "checkpoint"):
                    asyncio.run(MetaEvolutionRunner(changed, ROOT).run(fixture_meta_samples(), final_test_domains=final, resume=True, evaluation_only=True))
    ...
    def test_committed_run_resumes_without_recommit(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            first = asyncio.run(MetaEvolutionRunner(config, ROOT, repo).run(fixture_meta_samples(), final_test_domains=("outer_holdout",)))
            before = repo.active_file.read_bytes()
            second = asyncio.run(MetaEvolutionRunner(config, ROOT, repo).run(fixture_meta_samples(), final_test_domains=("outer_holdout",), resume=True))
...
class StatisticalRegressionTests(unittest.TestCase):
    def test_invalid_pairs_fail(self):
        row = SampleEvaluation("a", "REAL", "REAL", .8)
    ...
    def test_specialization_cannot_bypass_hard_constraints(self):
        base = TransferUtility("c", 3, ("a", "b"), .01, .2, (-.1, .1), .5, .5, -.05, 1, 0, 0, 1, {"a": .2, "b": -.05})
        gate = MetaValidationGate(MetaLearningConfig(enabled=True))
        for changes in ({"mean_coverage": .1}, {"cost_ratio": 100}, {"calibration_delta": 1}, {"episode_count": 2}):
