...
from evofact.generation.data import fixture_adversarial_data, validate_fact_links, load_samples
...
class FakeJSONBackend:
    ...
    async def _call(self, system, payload):
        ...
        if system == VERIFIER_SYSTEM:
            return {"reviews": [{"sample_id": s["sample_id"],
                                 "label": "REAL" if s["text"].startswith("记录确认") else "FAKE",
                                 "reason": "fixture evidence check"} for s in payload["samples"]]}
        ...
        for index in range(payload["batch_size"]):
            ...
            row["text"] = ("记录确认：" if row["label"] == "REAL" else "记录否认：") + source["evidence"][0]["text"] + f"（第{index}份转述）"
            rows.append(row)
            decisions.append({"sample_id": row["sample_id"], "source_sample_id": source["sample_id"],
                              "strategy": "success_boundary_extension" if example["outcome"] == "success" else "minimal_contrast",
                              "reason": "成功类型升难度" if example["outcome"] == "success" else "失败类型增样强化",
...
def config_for(directory):
    ...
    return replace(config, backend="mock", model="mock-v1", output_dir=directory / "reports",
                   skill_store=directory / "detector",
                   generation=replace(config.generation, batch_size=4, store_path=directory / "generator.json"),
...
class GenerationTests(unittest.TestCase):
    def setUp(self):
        ...
        traces[0] = replace(traces[0], decision=Prediction(_gold(self.samples[0].label), .9))
        traces[1] = replace(traces[1], decision=Prediction("REAL", .9))
        reports = [attribute_trace(t, _gold(s.label)) for t, s in zip(traces, self.samples[:4])]
        self.request = ChallengeGenerator().build_request(self.samples[:4], traces, reports,
                                                          config=self.config, episode_id="test")
    ...
    def test_free_text_survives_and_reviewer_is_blind(self):
        checked, rejected = ChallengeVerifier().validate(self.response, self.request, config=self.config,
        ...
        self.assertTrue(all(set(s) == {"sample_id", "text", "evidence"} for s in review_payload["samples"]))

    def test_provenance_schema_and_leakage_reject(self):
        for field, value in (("dataset", "held-out"), ("event_id", "unknown"), ("metadata", {"strategy": "secret"}),
                             ("label", "MAYBE"), ("text", ""), ("evidence", [])):
            response = deepcopy(self.response)
            response["samples"][0][field] = value
            accepted, rejected = ChallengeVerifier().validate(response, self.request, config=self.config)
            self.assertEqual(len(accepted), 3, field)
            self.assertTrue(rejected, field)
        for field, value in (("source_sample_id", "held-out"), ("source_trace_ids", ["held-out"]),
        ...
        _, rejected = ChallengeVerifier().validate(response, self.request, config=self.config, forbidden=self.samples[-2:])
        self.assertTrue(rejected)
        response = deepcopy(self.response)
        response["samples"][0]["text"] = self.samples[0].text
        _, rejected = ChallengeVerifier().validate(response, self.request, config=self.config, existing=self.samples)
        self.assertTrue(rejected)

    def test_bad_batch_rejected_without_retry(self):
        for response in ({}, {"samples": [], "decisions": [{}]},
    ...
    def test_unknown_or_wrong_label_not_used_for_training(self):
        ...
        class Reviewer:
            async def _call(self, system, payload):
                return {"reviews": [{"sample_id": s["sample_id"], "label": "UNKNOWN", "reason": "insufficient"}
        ...
        class WrongReviewer:
            async def _call(self, system, payload):
                return {"reviews": [{"sample_id": s.sample_id,
                                     ...
                                     "reason": "contradictory verdict"} for s in checked]}
    ...
    def test_split_preserves_groups_without_numeric_facts(self):
        ...
        construction, probe, facts = split_construction_probe(train, [], fraction=.4, seed="x")
        self.assertFalse({s.event_id for s in construction} & {s.event_id for s in probe})
        self.assertFalse({e.text for s in construction for e in s.evidence} & {e.text for s in probe for e in s.evidence})
        self.assertFalse(facts)
        self.assertEqual((construction, probe, facts), split_construction_probe(list(reversed(train)), [], fraction=.4, seed="x"))
        with self.assertRaisesRegex(ValueError, "two independent"):
            split_construction_probe(self.samples[:2], [], fraction=.4, seed="x")
        with self.assertRaises(ValueError):
    ...
    def test_source_snapshot_groups_even_when_evidence_wording_differs(self):
        ...
        for i in (0, 1, 2, 3):
            samples[i] = replace(samples[i], evidence=(replace(samples[i].evidence[0], source=shared,
                                                               text=f"不同段落 {i}"),))
        construction, probe, _ = split_construction_probe(samples, [], fraction=.4, seed="x")
        self.assertTrue({samples[i].sample_id for i in range(4)} <= {s.sample_id for s in construction}
    ...
    def test_real_backend_serializes_evidence_dates_without_network(self):
        ...
        with patch("evofact.runtime.openai_backend.urllib.request.urlopen") as request:
            request.return_value.__enter__.return_value.read.return_value = json.dumps({
                "choices": [{"message": {"content": '{"samples": [], "decisions": []}'}}]}).encode()
            asyncio.run(backend._call("test", {"published_at": datetime(2026, 1, 1)}))
            body = json.loads(request.call_args.args[0].data)
            self.assertEqual(json.loads(body["messages"][1]["content"])["published_at"], "2026-01-01 00:00:00")
...
class AdversarialRunnerTests(unittest.TestCase):
    def test_future_evidence_and_cross_domain_sources_rejected_before_calls(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            variants = [
                [replace(samples[0], published_at=when, evidence=(replace(samples[0].evidence[0],
                          published_at=when + timedelta(days=1)),)), *samples[1:]],
                [replace(samples[0], evidence=(replace(samples[0].evidence[0],
                          source=samples[-1].evidence[0].source),)), *samples[1:]],
            ]
            for rows in variants:
                backend = FakeJSONBackend()
                with self.assertRaisesRegex(ValueError, "future evidence|crosses adversarial"):
                    asyncio.run(AdversarialEvolutionRunner(config, ROOT, generation_backend=backend).run(
    ...
    def test_end_to_end_one_call_isolation_and_resume(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            outcome = asyncio.run(runner.run(samples, final_test_domains=("outer_holdout",), evaluation_only=True))
            ...
            for episode in outcome.episodes:
                ...
                self.assertEqual(set(record["construction_ids"]) | set(record["probe_ids"]), set(episode.meta_train_sample_ids))
            ...
            paths = write_adversarial_report(config.output_dir, outcome, runner.audit, config.generation)
            for target in paths["sample_files"].values():
                self.assertEqual(len(load_samples(target)), 4)
            restored = AdversarialEvolutionRunner(config, ROOT, facts, generation_backend=FakeJSONBackend())
            with patch.object(restored.generator, "generate", side_effect=AssertionError("completed episode repeated")):
                resumed = asyncio.run(restored.run(samples, final_test_domains=("outer_holdout",), resume=True, evaluation_only=True))
    ...
    def test_commit_audit_idempotent_no_policy_weights(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            runner = AdversarialEvolutionRunner(config, ROOT, facts, repo, generation_backend=FakeJSONBackend())
            ...
            restored = AdversarialEvolutionRunner(config, ROOT, facts, repo, generation_backend=FakeJSONBackend())
    ...
    def test_changed_generation_budget_cannot_resume(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            config = config_for(Path(temp))
            asyncio.run(AdversarialEvolutionRunner(config, ROOT, facts, generation_backend=FakeJSONBackend()).run(
            ...
            with self.assertRaisesRegex(ValueError, "checkpoint"):
                asyncio.run(AdversarialEvolutionRunner(changed, ROOT, facts, generation_backend=FakeJSONBackend()).run(
                    samples, final_test_domains=("outer_holdout",), resume=True, evaluation_only=True))
    ...
    def test_cli_samples_without_facts_and_no_mock_fallback(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            sample_path.write_text("\n".join(json.dumps(asdict(s), ensure_ascii=False) for s in samples), encoding="utf-8")
            args = build_parser().parse_args(["adversarial-evolve", "--samples", str(sample_path), "--evaluation-only"])
            with patch("evofact.cli.load_config", return_value=config), patch.object(ExperimentRunner, "_backend", wraps=None) as unused:
            ...
            with patch("evofact.cli.load_config", return_value=config), patch.object(AdversarialEvolutionRunner, "__init__", injected):
    ...
    def test_interrupt_after_generation_reuses_response(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch.object(runner.verifier, "verify", side_effect=RuntimeError("injected interruption")):
                with self.assertRaisesRegex(RuntimeError, "injected"):
                    asyncio.run(runner.run(samples, final_test_domains=("outer_holdout",), evaluation_only=True))
            ...
            asyncio.run(restored.run(samples, final_test_domains=("outer_holdout",), resume=True, evaluation_only=True))
            self.assertEqual(sum(s == GENERATOR_SYSTEM for s, _ in backend.calls), 5)
